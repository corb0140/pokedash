...
export default function PokemonDetailModal({
  ...
  // CONTENT SLIDE LEFT RIGHT ANIMATION
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <span
        onClick={handleClose}
        className="sticky top-5 left-100 text-lg z-10"
      >
      ...
      <div ref={contentRef} key={selectedId}>
        {/* IMAGE */}
        ...
        <div className="mt-5">
          ...
          <div className="flex flex-wrap gap-3">
            {pokemonData?.abilities.map((a: any, index: number) => (
              <span
                key={a}
                className={`capitalize flex items-center justify-center font-bold grow bg-page-background px-5 py-1.5 border rounded-lg text-sm ${index % 2 === 0 ? 'border-link' : 'border-ability-border'}`}
              >
                {a}
              </span>
            ))}
              ...
              <div key={evo.name} className="flex items-center gap-4">
                ...
                {index < pokemonData.evolutionChain.length - 1 && (
                  <div className="flex flex-col items-center text-xs font-semibold text-link">
                    <span>
                      Lv.{' '}
                      {pokemonData.evolutionChain[index + 1].minLevel ?? '?'}
                    </span>
                  </div>
                )}
              </div>
        ...
        {/* PREVIOUS & NEXT */}
        <div className="mt-8 flex items-center justify-center min-h-20 w-full gap-6 bg-page-background rounded-lg">
          <button
            onClick={handlePrev}
          ...
          <div className="border-r border-r-black/40 min-h-10"></div>
